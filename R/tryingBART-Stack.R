...
bart_bma[[q]] <- bartBMA.default(x.train = xcov, y.train=y, x.test=xcovtest)
while(length(bart_bma[[q]]$sumoftrees)==1){
  ...
  xcov <- as.matrix(train[,-1]); y <- as.vector(train$y)
  xcovtest <- as.matrix(test[,-1])
  bart_bma[[q]] <- bartBMA(x.train = xcov, y.train=y, x.test=xcovtest)
}
foo <- pred_intervals(bart_bma[[q]], num_iter = 5000, burnin = 1000, l_quant = .25, u_quant = .75)
...
parsed <- parseTreeSums(bart_bma[[q]])
...
b <- exp(bart_bma[[q]]$bic)/sum(exp(bart_bma[[q]]$bic))

stackwts[[q]] = wts
bmawts[[q]] = b

sizes[q] <- length(loolist)
bmapred <- predict.bartBMA(bart_bma[[q]], newdata = xcovtest)
stackpred <- predict.bartStack(bart_bma[[q]], newdata = xcovtest)
...
rf <- predict(randomForest(x = xcov, y = y, ntree = length(bart_bma[[q]]$sumoftrees)), newdata = xcovtest)
mse4[q] <- mse(ytest, rf)
print(q)
}
end.time <- Sys.time()
...
w=2
for(w in 1:length(stackwts)){
  sumtemp = bart_bma[[w]]$sumoftrees
  sumotree = sumtemp[which(as.numeric(stackwts[[w]]) > .1)]
  a = varIncProb(bart_bma[[w]],score = stackwts[[w]],numvars = bart_bma[[w]]$numvars,sumoftrees=sumotree)
  print(a[which(a>0)])
}
w=2
for(w in 1:length(bmawts)){
  sumtemp = bart_bma[[w]]$sumoftrees
  sumotree = sumtemp[which(as.numeric(bmawts[[w]]) > .1)]
  a=varIncProb(bart_bma[[w]],score = bmawts[[w]],numvars = bart_bma[[w]]$numvars,sumoftrees=sumotree)
  print(a[which(a>0)])
}


which(varImpScores(bart_bma[[1]], scores = stackwts[[1]])!=0)
which(varImpScores(bart_bma[[1]], scores = bmawts[[1]])!=0)

which(varImpScores(bart_bma[[2]], scores = stackwts[[2]])!=0)
which(varImpScores(bart_bma[[2]], scores = bmawts[[2]])!=0)

varIncProb(bart_bma[[2]], score = stackwts[[2]])
varIncProb(bart_bma[[2]], score = bmawts[[2]])
